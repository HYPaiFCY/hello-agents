...
class MarkItDownPipelineDemo:
    ...
    def __init__(self):
        self.rag_tool = RAGTool(
            knowledge_base_path="./demo_rag_kb", rag_namespace="markitdown_demo"
        )
...
知识图谱,技术组件,0.9,实体关系的结构化表示"""
    ...
    def demonstrate_markitdown_conversion(self, file_paths):
        ...
        for filename, file_path in file_paths.items():
            ...
            conversion_results[filename] = {"result": result, "time": process_time}
...
"""
        ...
        print(f"\n🔍 测试预处理后的检索效果:")
        search_result = self.rag_tool.run(
            {"action": "search", "query": "Python函数处理数据", "limit": 1}
        )
