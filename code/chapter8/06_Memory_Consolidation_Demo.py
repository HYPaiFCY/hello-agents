...
class MemoryConsolidationDemo:
    ...
    def demonstrate_consolidation_process(self):
        ...
        for threshold, description in consolidation_tests:
            ...
            # 查看整合后的情景记忆
            print(f"\n📚 整合后的情景记忆:")
            episodic_search = self.memory_tool.run(
                {"action": "search", "query": "", "memory_type": "episodic", "limit": 5}
            )
    ...
    def demonstrate_consolidation_metadata(self):
        ...
        # 添加一个特殊的工作记忆用于演示
        special_memory_result = self.memory_tool.run(
            {
                "action": "add",
    ...
    def demonstrate_consolidation_benefits(self):
        ...
        for memory_type in memory_types:
            ...
            type_summary = self.memory_tool.run(
                {
                    "action": "search",
                    "query": "",
                    "memory_type": memory_type,
