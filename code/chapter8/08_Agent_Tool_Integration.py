...
class AgentIntegrationDemo:
    ...
    def setup_agent(self):
        ...
        self.rag_tool = RAGTool(
            knowledge_base_path="./agent_integration_kb", rag_namespace="agent_demo"
        )
...
"""
        ...
        memory_search = self.memory_tool.run(
            {"action": "search", "query": "设计模式学习", "limit": 3}
        )
        print(f"学习历史回顾: {memory_search}")

        # 从RAG获取相关知识补充
        rag_search = self.rag_tool.run(
            {"action": "search", "query": "观察者模式", "limit": 2}
        )
...
"""
        ...
        experience_search = self.memory_tool.run(
            {"action": "search", "query": "学习计划 学习经验", "limit": 3}
        )
    ...
    def demonstrate_performance_analysis(self):
        ...
        for i in range(3):
            self.rag_tool.run(
                {"action": "search", "query": f"测试查询 {i+1}", "limit": 2}
            )
        ...
        rag_result = self.rag_tool.run(
            {"action": "search", "query": "性能测试", "limit": 1}
        )
