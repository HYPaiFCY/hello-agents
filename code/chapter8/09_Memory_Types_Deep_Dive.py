...
class MemoryTypesDeepDive:
    ...
    def setup_memory_systems(self):
        ...
        self.working_memory_tool = MemoryTool(
            user_id="working_memory_user", memory_types=["working"]
        )

        self.episodic_memory_tool = MemoryTool(
            user_id="episodic_memory_user", memory_types=["episodic"]
        )

        self.semantic_memory_tool = MemoryTool(
            user_id="semantic_memory_user", memory_types=["semantic"]
        )

        self.perceptual_memory_tool = MemoryTool(
            user_id="perceptual_memory_user", memory_types=["perceptual"]
        )
    ...
    def demonstrate_working_memory(self):
        ...
        for query in search_queries:
            ...
            results = self.working_memory_tool.run(
                {
                    "action": "search",
                    "query": query,
                    "memory_type": "working",
    ...
    def demonstrate_episodic_memory(self):
        ...
        # 按时间顺序检索
        timeline_search = self.episodic_memory_tool.run(
            {
                "action": "search",
                "query": "学习",
                "memory_type": "episodic",
        ...
        # 按会话检索
        session_search = self.episodic_memory_tool.run(
            {
                "action": "search",
                "query": "线性回归",
                "memory_type": "episodic",
        ...
        context_result = self.episodic_memory_tool.run(
            {
                "action": "add",
        ...
        # 检索整个链条
        chain_search = self.episodic_memory_tool.run(
            {
                "action": "search",
    ...
    def demonstrate_semantic_memory(self):
        ...
        for query in semantic_queries:
            start_time = time.time()
            results = self.semantic_memory_tool.run(
                {
                    "action": "search",
                    "query": query,
                    "memory_type": "semantic",
    ...
    def demonstrate_perceptual_memory(self):
        ...
        for query, description in cross_modal_queries:
            results = self.perceptual_memory_tool.run(
                {
                    "action": "search",
                    "query": query,
                    "memory_type": "perceptual",
        ...
        # 分析不同模态的分布
        modality_analysis = self.perceptual_memory_tool.run(
            {
                "action": "search",
                "query": "模态分析",
                "memory_type": "perceptual",
    ...
    def demonstrate_memory_interactions(self):
        ...
        # 从工作记忆整合到情景记忆
        consolidation_result = self.working_memory_tool.run(
            {
                "action": "consolidate",
                "from_type": "working",
                "to_type": "episodic",
