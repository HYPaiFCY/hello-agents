...
def memory_tool_execute_demo():
    ...
    print(
        f"📋 支持的操作: add, search, summary, stats, update, remove, forget, consolidate, clear_all"
    )
...
def search_memory_demo(memory_tool):
    ...
    print("\n按类型搜索 - 语义记忆中的'记忆':")
    result = memory_tool.run(
        {"action": "search", "query": "记忆", "memory_type": "semantic", "limit": 2}
    )
    print(result)
    ...
    print("\n高重要性记忆搜索:")
    result = memory_tool.run(
        {"action": "search", "query": "AI Agent", "min_importance": 0.7, "limit": 3}
    )
    print(result)
